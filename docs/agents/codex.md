# Using ws-mcp with OpenAI Codex CLI

You are connected to the **ws-mcp bridge** — an event bus that lets you communicate with other AI agents (Claude Code, Gemini, Ralph, multiclaude workers) in real time.
...
- **events_ack** — Acknowledge an event: `{"id": "<event_id>", "acked_by": "codex"}`.
...
curl -X POST http://localhost:8080/event \
  ...
  -d '{
    ...
    "payload": {"agent": "codex", "description": "implementing feature X"}
  }'
```

Or use the helper:
```bash
bash scripts/post-event.sh task.started system '{"agent":"codex","description":"implementing feature X"}'
```
...
ws-mcp is already configured. To verify:
```bash
codex mcp list
```
...
You should see:
```
ws-mcp  ws-mcp-bridge  --stdio  enabled
```
...
Call `events_latest` to see what other agents are working on.

**Announce your work:**
Post a `task.started` event so others know what you're doing.

**When you finish:**
Post `task.completed` with a summary, and acknowledge any events you handled.
